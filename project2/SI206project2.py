...
def find_urls(s):
    urls = re.findall('(http[s]?://\S+\.[a-zA-Z0-9]{2,})',s)
    return urls
...
def get_umsi_data():
    ...
    for i in range(13):
        ...
        if i == 0:
            ...
            r = requests.get(base_url + ("&page={}".format(i)), headers={'User-Agent': 'SI_CLASS'})
        soup = BeautifulSoup(r.text,"lxml")
        si_people = soup.find_all("div", "field-name-title")

        for person in si_people:
            try:
                people.append(person.h2.text)
            except:
                continue

        si_roles = soup.find_all(class_ = "field-name-field-person-titles")
        for role in si_roles:
            roles.append(role.text)

    umsi_titles = dict(zip(people,roles))
    return umsi_titles
...
def num_students(data):
    #use regex to find all PhD students
    phd = [x for x in data.values() if "PhD" in x]
    return len(phd)
